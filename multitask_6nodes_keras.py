...
data = pd.read_csv('6nodesReg/result_n=6_200000sample.csv', header=-1, delimiter='\t')
#labels = pd.read_csv('encoded_flabels.csv', header=-1)
print (np.array(data).shape)

X = np.array(data)[:,0:-7]

Y1 = np.atleast_2d(np.array(data)[:,-6]).T
...
for i in range(1):
    ...
    model = Sequential()
    model.add(Dense(60, input_dim=36, activation='relu', kernel_initializer='he_uniform'))
    #model.add(BatchNormalization())
    ...
    beg_tr = time.time()
    model.fit(features, labels, epochs=30, verbose=1)
    end_tr = time.time()
...
##class_pred.to_csv("6nodesReg/class_prediction_6nodes_30epochs_reg_th015.csv")
##class_label.to_csv("6nodesReg/class_label_6nodes_30epochs_reg_th015.csv")
##reg_pred.to_csv("6nodesReg/reg_prediction_6nodes_30epochs_reg_th015.csv")
##reg_label.to_csv("6nodesReg/reg_label_6nodes_30epochs_reg_th015.csv")
##result.to_csv("6nodesReg/results_30epochs_6nodes_reg_th015.csv")
...
with open("6nodesReg/model.json", "w") as json_file:
