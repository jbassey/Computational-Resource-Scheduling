...
data = pd.read_csv('3nodes/result_test_n=3_50000.csv', header=-1)#, delimiter='\t')
#labels = pd.read_csv('encoded_flabels.csv', header=-1)
print (np.array(data).shape)

X = np.array(data)[:,0:-4]

Y1 = np.atleast_2d(np.array(data)[:,-4]).T
...
for i in range(3):
    ...
    model = Sequential()
    model.add(Dense(60, input_dim=18, activation='relu', kernel_initializer='he_uniform'))
    #model.add(BatchNormalization())
...
class_pred.to_csv("3nodes/class_prediction_3nodes_30epochs_reg_th015.csv")
class_label.to_csv("3nodes/class_label_3nodes_30epochs_reg_th015.csv")
reg_pred.to_csv("3nodes/reg_prediction_3nodes_30epochs_reg_th015.csv")
reg_label.to_csv("3nodes/reg_label_3nodes_30epochs_reg_th015.csv")
result.to_csv("3nodes/results_30epochs_reg_th015.csv")
...
with open("3nodes/model.json", "w") as json_file:
