...
data = pd.read_csv('4nodes/result_test_n=4_80000.csv', header=-1)#, delimiter='\t')
#labels = pd.read_csv('encoded_flabels.csv', header=-1)
print (np.array(data).shape)

X = np.array(data)[:,0:-5]

Y1 = np.atleast_2d(np.array(data)[:,-5]).T
...
for i in range(3):
    ...
    model = Sequential()
    model.add(Dense(60, input_dim=24, activation='relu', kernel_initializer='he_uniform'))
    #model.add(BatchNormalization())
...
class_pred.to_csv("4nodes/class_prediction_4nodes_30epochs_reg_th015.csv")
class_label.to_csv("4nodes/class_label_4nodes_30epochs_reg_th015.csv")
reg_pred.to_csv("4nodes/reg_prediction_4nodes_30epochs_reg_th015.csv")
reg_label.to_csv("4nodes/reg_label_4nodes_30epochs_reg_th015.csv")
result.to_csv("4nodes/results_30epochs_reg_th015.csv")
