...
data = pd.read_csv('8nodes/result_n=8_300000sample.csv', header=-1)#, delimiter='')
#labels = pd.read_csv('encoded_flabels.csv', header=-1)
print (np.array(data).shape)

X = np.array(data)[:,0:-9]

Y1 = np.atleast_2d(np.array(data)[:,-8]).T
...
for i in range(3):
    ...
    model = Sequential()
    model.add(Dense(60, input_dim=48, activation='relu', kernel_initializer='he_uniform'))
    #model.add(BatchNormalization())
    ...
    classifier = np.where(prediction < 0.1, 0, 1)
    X_test_class = np.where(y_test < 0.1, 0, 1)
...
class_pred.to_csv("8nodes/class_prediction_8nodes_50epochs_reg_th01.csv")
class_label.to_csv("8nodes/class_label_8nodes_50epochs_reg_th01.csv")
reg_pred.to_csv("8nodes/reg_prediction_8nodes_50epochs_reg_th01.csv")
reg_label.to_csv("8nodes/reg_label_8nodes_50epochs_reg_th01.csv")
result.to_csv("8nodes/results_50epochs_8nodes_reg_th01.csv")
...
with open("8nodes/model.json", "w") as json_file:
